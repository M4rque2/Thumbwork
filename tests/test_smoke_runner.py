...
from thumbwork.smoke_runner import default_scenario_paths, load_scenario, run_scenario


from thumbwork.workspace import RESOURCE_ROOT

SYSTEM_PROMPT = (RESOURCE_ROOT / "system_prompt.md").read_text(encoding="utf-8").strip()
...
class FakeVlm:
    ...
    def invoke(self, messages):
        ...
        return LLMResult(self.outputs.pop(0))


class SmokeRunnerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.paths = default_scenario_paths()
    ...
    def test_misplaced_click_reports_actual_position_and_target(self):
        ...
        for task, coordinate, pixel, bounds in cases:
            with self.subTest(task=task):
                scenario = load_scenario(self.paths[task])
                fake = FakeVlm([_response({"action": "click", "coordinate": coordinate})])
                result = run_scenario(scenario, SYSTEM_PROMPT, fake)
                self.assertFalse(result["passed"])
                self.assertEqual(result["failure_category"], "action_match")
                self.assertIn(pixel, result["message"])
                self.assertIn(bounds, result["message"])
                self.assertIn("1080×2424", result["message"])
