...
def _click_miss_details(arguments: dict[str, Any], scenario: SmokeScenario, rect_px: list[Any]) -> str:
    if arguments.get("action") != "click":
        return ""
    coordinate = arguments.get("coordinate")
    if not isinstance(coordinate, (list, tuple)) or len(coordinate) != 2:
        return "; click must provide [x, y] coordinates in the 0–1000 range"
    try:
        x, y = (float(value) for value in coordinate)
    except (TypeError, ValueError):
        return "; click coordinates must be numbers in the 0–1000 range"
    if not (0 <= x <= 1000 and 0 <= y <= 1000):
        return f"; click {coordinate} is outside the required 0–1000 coordinate range"
    left, top, right, bottom = rect_px
    return (
        f"; click [{x:g}, {y:g}] (0–1000 coordinates) landed at pixel "
        f"({x * scenario.width / 1000:.1f}, {y * scenario.height / 1000:.1f}) "
        f"on the {scenario.width}×{scenario.height} screenshot, outside the target "
        f"pixel bounds x=[{left}, {right}], y=[{top}, {bottom}]"
    )
...
def _check_single_turn(scenario: SmokeScenario, arguments: dict[str, Any]) -> tuple[bool, str]:
    ...
    if kind == "open_or_click":
        ...
        return False, ("expected open(com.sina.weibo) or a click inside the Weibo icon bounds"
                       + _click_miss_details(arguments, scenario, scenario.oracle["click_region_px"]))
...
def run_scenario(
    ...
    *,
    ...
    for turn in range(1, scenario.max_turns + 1):
        def build(current):
            return _build_turn_messages(scenario, system_prompt, current["history"], previous_expectation, current["summary"])
        try:
            messages = manager.prepare(state, build) if manager else build(state)
            try:
                reply = vlm.invoke(messages)
            except ContextOverflowError:
                if manager is None: raise
                messages = manager.prepare(state, build, force=True)
                reply = vlm.invoke(messages)
            if manager: manager.counter.observe(messages, reply.usage)
...
def default_scenario_paths(root: str | Path | None = None) -> dict[str, Path]:
    root_path = Path(root).resolve() if root else Path(__file__).parent / "resources" / "smoke"
    return {
